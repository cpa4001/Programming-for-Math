...
This module covered the three basic problems in Integral and Differeintal Calculus, allowing me to learn limits, derivatives, integrals. When using sage for Calculus, it is important to define functions and then perform calculations. Starting with Limits, we are trying to find the function value at a certain x-value and sage can help us compute this; however, sage has a small problem when finding limits at discontinuities and this forces the user to use directional limits. We can also solve derivatives and derivatives at certain points with sage which can also bring us to finding the tangent line to a function curve at a certain point. 
